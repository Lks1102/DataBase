...
SELECT a.`treat_no` 진료번호,
		 e.`pat_name` 환자이름,
		 b.`doc_name` 의사이름,
		 a.`treat_contents` 진료내용,
		 d.`chart_contents` 차트내용,
		 a.`treat_datetime`
FROM `treatments`  AS a
JOIN `doctors` 	 AS b ON a.doc_id = b.doc_id
JOIN `departments` AS c ON b.dep_no = c.dep_no
...
#문제7 현재날짜를 기준으로 30이상 40미만 환자를 조회
...
WHERE (YEAR(NOW()) - CONCAT(19,SUBSTRING(`pat_jumin`, 1, 2))+1) BETWEEN 30 and 40;



#문제8 모든 전공부서에 아직 의사ID가 부여되지 않은 의사의 이름과 부서명 조회

SELECT a.`dep_manager` 부서장,
		 a.`dep_name` 부서명
FROM `departments`  AS a
LEFT JOIN `doctors` AS b ON a.dep_no = b.dep_no
WHERE b.doc_id IS NULL;

#문제9 김태희 간호사가 맡고 있는 모든 환자의 이름을 구분자^으로 해서 '강동원^고소영^이나영'과 같이 조회

SELECT
	GROUP_CONCAT(`pat_name` SEPARATOR '^') AS 환자이름
FROM `patients` AS a
JOIN `nurses`	 AS b ON a.nur_id = b.nur_id
WHERE b.nur_name = '김태희';


#문제10 가장 많은 환자 수를 담당하는 간호사 ID, 간호사 이름, 담당환자수 조회

SELECT b.`nur_id` 간호사ID,
		 b.`nur_name` 이름,
		 COUNT(*) AS 담당환자
FROM `patients` AS a
JOIN `nurses`	 AS b ON a.nur_id = b.nur_id
